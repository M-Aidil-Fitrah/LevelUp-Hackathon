    ...
    <div className="min-h-screen bg-gray-50 px-4 md:px-8 py-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl md:text-2xl font-bold">Dashboard Seller</h1>
          <button onClick={() => navigate('/marketplace')} className="text-sm px-3 py-1.5 rounded-md border hover:bg-gray-100">Ke Marketplace</button>
        </div>

        {/* Form Add/Edit */}
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-4 md:p-5 mb-6 grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-4">
          ...
          <div className="md:col-span-4 flex gap-3 justify-end">
            {editingId && (
              <button type="button" onClick={resetForm} className="px-4 py-2 border rounded-md text-sm">Batal</button>
            )}
        ...
        <div className="bg-white rounded-xl shadow divide-y">
          {products.length === 0 ? (
            <div className="p-6 text-sm text-gray-500">Belum ada produk. Tambahkan produk pertama Anda.</div>
          ) : (
            products.map(p => (
              <div key={p.id} className="p-4 flex items-start gap-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(p.id)} className="px-3 py-1.5 border rounded-md text-sm">Edit</button>
