...
const MenuItem: React.FC<MenuItemProps> = ({ link, text, image }) => {
  ...
  const repeatedMarqueeContent = React.useMemo(() => {
    return Array.from({ length: 4 }).map((_, idx) => (
      <React.Fragment key={idx}>
        <span className="text-white uppercase font-semibold text-[4vh] leading-[1.2] p-[1vh_1vw_0]">{text}</span>
        <div
          className="w-[200px] h-[7vh] my-[2em] mx-[2vw] p-[1em_0] rounded-[50px] bg-cover bg-center ring-1 ring-white/40"
  ...
  return (
    <div className="relative overflow-hidden text-center bg-white group border-t border-neutral-200 h-24 md:h-28" ref={itemRef}>
      ...
      <div
        className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none bg-[#FF2000] translate-y-[101%]"
        ref={marqueeRef}
      >
