...
const PillNav: React.FC<PillNavProps> = ({
  ...
  return (
    <div
      ...
      style={{ ...(cssVars as React.CSSProperties), marginBottom: 'calc(var(--nav-h) * -1)' }}
    >
      <nav
        ...
        {isRouterLink(items?.[0]?.href) ? (
          <Link
            ...
            style={{
              width: 'var(--nav-h)',
              height: 'var(--nav-h)',
              background: logoBgColor || 'var(--base, #000)'
            }}
          >
            <img src={logo} alt={logoAlt} ref={logoImgRef} className="w-full h-full object-cover object-center block" />
          </Link>
        ) : (
          <a
            href={items?.[0]?.href || '#'}
        ...
        <ul className="list-none m-0 p-[3px] flex flex-col gap-[3px]">
          {items.map(item => {
            ...
            const hoverIn = (e: React.MouseEvent<HTMLAnchorElement>) => {
              // Hover tetap hitam untuk konsistensi dengan desktop bar
